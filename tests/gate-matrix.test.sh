...
if [ "$UPDATE" -eq 1 ] || [ "$MISSING_GOLDEN" -eq 1 ]; then
  mkdir -p "$(dirname "$GOLDEN")"
  {
    ...
    echo "#                           of them legitimately acquired the lock and would"
    echo "#                           then leak it. The correct owner-tracking fix"
    echo "#                           flips exactly ONE: gate12. Six diffs here means"
    echo "#                           the naive fix was taken, not a broken baseline."
    echo "#"
