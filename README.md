...
- **`## Open Questions` signals reviewer disagreement, and the heading has three outcomes, not two** — Gate 11 stops the loop when a brief's `## Open Questions` section holds a real item, because a disagreement between the two reviewers is the one thing the loop must not resolve on its own. Items may be `-`/`*`/`+` bullets or numbered (`1.`, `1)`); placeholder items (`- none`, `- n/a`, `- 없음`) do not count, and fenced code blocks are skipped so a brief may quote an example safely. The heading match is anchored to end-of-line, so a *suffixed* or decorated heading — `## Open Questions (unscored)`, `## Open Questions [DEGRADED]`, `## ❓ Open Questions`, `## Open questions` — does **not** terminate the loop: reviewers use those for their own notes, and terminating on them was measured as a net regression. It does **not** silently advance either. Such a heading, and an exact heading whose body is prose or a table rather than a list, put the loop into a third state: **it pauses with state preserved** and the message names the heading it saw. To clear it, rename the heading to exactly `## Open Questions` with top-level bullets if it is a real decision, or to anything else if it is a note. This is deliberately fail-closed — before it existed, every near miss resolved toward the loop settling a disagreement by itself.
