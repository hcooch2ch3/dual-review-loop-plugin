...
- **`## Open Questions` signals reviewer disagreement, and the heading has three outcomes** — Gate 11 ends the loop when a brief's `## Open Questions` section holds a real item, because a disagreement between the two reviewers is the one thing the loop must not resolve on its own. Items may be `-`/`*`/`+` bullets or numbered (`1.`, `1)`); placeholders (`- none`, `- n/a`, `- 없음`, `- 없다`) do not count, and fenced blocks are skipped so a brief may quote an example safely. A heading that *begins with* the phrase but is not exactly `## Open Questions` — `## Open Questions (unscored)`, `## Open Questions [DEGRADED]`, `## ❓ Open Questions` — does **not** end the loop (reviewers use those for their own notes, and terminating on them was measured as a regression) and does **not** silently advance: the loop **pauses with state preserved** and names the heading it saw. Rename it to anything not beginning with `Open Questions` and the loop resumes; rename it to exactly `## Open Questions` and the loop ends and hands you the decision. Until then the message repeats each turn, and at 24h the loop is collected — with a message that says it was held, not idle. Deliberately narrow: only the *heading* is classified. A prose or table body under an exact heading advances, because heading shapes are what was measured and body shapes produced mostly false positives — including this project's own `없다. …` house style. Headings that merely mention the phrase (`## No Open Questions`, `### Task 3: Open Questions …`) are not matched.
