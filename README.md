...
- **`## Open Questions` signals reviewer disagreement, and only that exact heading counts** — Gate 11 stops the loop when a brief's `## Open Questions` section holds a real item, because a disagreement between the two reviewers is the one thing the loop must not resolve on its own. The heading match is anchored to end-of-line on purpose, so a *suffixed* heading — `## Open Questions (unscored)`, `## Open Questions → resolved` — is deliberately **not** matched; reviewers use those for their own notes, and removing the anchor was measured as a net regression (the loop began stopping on notes instead of on questions). Fenced code blocks are skipped, and placeholder items (`- none`, `- n/a`, `- 없음`) do not count either. To make the loop stop, write the question as a top-level bullet under a bare `## Open Questions`.
